...
const Shop = () => {
  ...
  const getActiveFilters = () => {
    ...
    // Categories
    filters.categories.forEach(cat => active.push({ type: 'category', value: cat }));
    
    // Colors
    filters.colors.forEach(color => active.push({ type: 'color', value: color }));
    ...
    if (filters.priceRange[0] > minPrice || filters.priceRange[1] < maxPrice) {
      active.push({ 
        type: 'price', 
        value: `$${filters.priceRange[0].toFixed(2)} - $${filters.priceRange[1].toFixed(2)}` 
      });
    }
        ...
        <div className="mb-8 text-center">
          ...
          {!queryParam && (
            <p className="text-gray-600 max-w-2xl mx-auto">
              Browse our curated collection of premium products, carefully selected for quality and style.
            </p>
          )}
        </div>
